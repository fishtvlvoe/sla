...
4. 產出規劃文件：
   ...
   - 技術架構說明
   - Epic 拆解
   - Stories 清單（每個 Story 標記會動哪些檔案）
    ↓
4.5. 並行分析（自動）：
   - 檢查各 Epic 之間是否有共用檔案
   - 無重疊 + 每個 Epic >= 3 個 Stories → 標記為「可並行」
   - 有重疊或任務太小 → 標記為「循序執行」
   - 在 PLAN.md 寫入 execution_mode: parallel | sequential
    ↓
5. gsd:plan-phase → 寫入 PLAN.md
    ↓
6. 詢問用戶確認，確認後自動路由：
   - execution_mode: parallel → cli-runner 建 worktree 並行執行各 Epic
   - execution_mode: sequential → /sla:develop 循序執行
```
...
## Epic 拆解
- Epic 1: [名稱] | files: [會動的檔案清單]
  - Story 1.1: [具體任務]
  - Story 1.2: [具體任務]
- Epic 2: [名稱] | files: [會動的檔案清單]
  ...
